...
function topicEditorsHTML(p, busy) {
  if (!p.topics?.length) return `<label>Краткое содержание<textarea id="summary" class="summary-area" ${busy ? "disabled" : ""}>${esc(p.summary)}</textarea></label>`;
  return p.topics.map((topic,i) => `<section class="topic-editor" data-topic-edit="${esc(topic.id)}"><h3>Тема ${i+1}</h3><label>Название темы<input data-topic-title maxlength="200" value="${esc(topic.title)}" ${busy ? "disabled" : ""}></label><label>Саммари темы<textarea data-topic-summary class="summary-area" ${busy ? "disabled" : ""}>${esc(topic.summary)}</textarea></label></section>`).join("");
}
function groupedTasksHTML(p, m) {
  if (!p.topics?.length) return p.tasks.map((t,i) => taskHTML(t,i,m)).join("");
  const groups = [...p.topics, {id:null,title:"Без темы"}];
  return groups.map(topic => {
    const rows = p.tasks.map((t,i) => ({t,i})).filter(({t}) => (t.topic_id || null) === topic.id);
    return rows.length ? `<h3 class="topic-task-heading">${esc(topic.title)} · ${rows.length}</h3>${rows.map(({t,i}) => taskHTML(t,i,m)).join("")}` : "";
  }).join("");
}
